...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    wget --no-check-certificate https://raw.githubusercontent.com/fgy4399/kuaij/main/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
...
————————————
 ...
 ${Green_font_prefix}7.${Font_color_suffix} 状态 gost
————————————
 ${Green_font_prefix}8.${Font_color_suffix} 新增gost转发配置
 ${Green_font_prefix}9.${Font_color_suffix} 查看现有gost配置
 ${Green_font_prefix}10.${Font_color_suffix} 删除一则gost配置
————————————
